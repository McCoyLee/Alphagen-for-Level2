...
def build_chat_client(log_dir: str) -> ChatClient:
    ...
    return OpenAIClient(
        client=OpenAI(
            base_url="http://10.2.1.205:8796/v1",        # ← 改成你的URL
            api_key="sk-GEmM5YHREocL6mOOVEOUQ0Rs0qgWoB_KjJ-fSZUYd30"      # ← 也可以直接写在这里（不推荐）
        ),
        config=ChatConfig(
            system_prompt=EXPLAIN_WITH_TEXT_DESC,
            logger=logger
        ),
        model="MiniMax-M2.5"          # ← 如果模型名不同也要改
    )
...
def run_single_experiment(
    ...
    segments = [
    ("2009-01-01", "2018-12-31"),  # 训练集
    ("2019-01-01", "2019-12-31"),  # 验证集
    ("2020-01-01", "2021-12-31"),  # 测试集
    ]
