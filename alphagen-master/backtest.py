...
if __name__ == "__main__":
    ...
    for p in Path("out/gp").iterdir():
        ...
        exprs_raw = state["exprs"]
        weights_raw = state["weights"]
        # 过滤掉解析失败的表达式
        exprs, weights = [], []
        for e, w in zip(exprs_raw, weights_raw):
            try:
                exprs.append(parse_expression(e))
                weights.append(w)
            except Exception as ex:
                print(f"跳过无法解析的GP表达式: {e} | 原因: {ex}")
        if exprs:
            run_backtest("gp", seed, exprs, weights)
    '''
    '''
    for p in Path("out/results").iterdir():
        inst, size, seed, time, ver = p.name.split('_', 4)
        size, seed = int(size), int(seed)
        if inst != "csi300" or size != 10 :
            continue
        pool_files = sorted(p.glob("*_pool.json"), key=lambda x: int(x.stem.split("_")[0]))
        if not pool_files:
            print(f"跳过 {p.name}：无 pool 文件")
            continue
        exprs, weights = load_alpha_pool_by_path(str(pool_files[-1]))
        run_backtest(ver, seed, exprs, weights)
    '''
    for i, p in enumerate(sorted(Path("out/llm-tests/interaction").iterdir())):
        if not (p / "report.json").exists():
            continue
        with open(p / "report.json") as f:
            report = json.load(f)
        state = report[-1]["pool_state"]
        run_backtest("pure_llm", i, [parse_expression(t[0]) for t in state], [t[1] for t in state])